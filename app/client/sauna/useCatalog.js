...
export function useCatalog() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([
      fetch('/assets/data/catalog.json').then(r => { if (!r.ok) throw new Error('catalog'); return r.json(); }),
      fetch('/assets/data/presets.json').then(r => { if (!r.ok) throw new Error('presets'); return r.json(); }),
