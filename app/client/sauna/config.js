...
export function normalizeConfig(cfg, catalog) {
  ...
  // Keep partially-filled room dimensions while the customer is still typing
  // (each field is entered one at a time) - only drop genuinely bad data.
  if (next.room) {
    const valid = ['widthCm', 'depthCm', 'heightCm'].every(k => Number.isFinite(next.room[k]) && next.room[k] >= 0 && next.room[k] <= 2000);
    next.room = valid ? next.room : null;
  }
  return next;
}
