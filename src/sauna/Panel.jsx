    ...
    <div className="opt-list">
      {Object.entries(catalog.heaters).map(([key, spec]) => (
        <Swatch
          ...
          title={nameOf(spec).split(', incl.')[0]}
          subtitle={heaterSpecLine(spec)}
          flag={fitsVolume(spec, volumeM3) ? '' : 'not rated for this cabin size'}
          price={spec.price}
  ...
  return (
    <div className="opt-list">
      <Swatch
        ...
        title="No separate control unit"
        subtitle="For heaters with built-in controls"
        price={0}
      ...
      {Object.entries(catalog.controls).filter(([k]) => k !== 'none').map(([key, spec]) => (
        <Swatch
          ...
          title={nameOf(spec).replace('HUUM UKU 4.2 control, ', 'HUUM UKU 4.2 · ')}
          subtitle={controlSpecLine(spec)}
          price={spec.price}
    ...
    <div className="panel-sections">
        ...
        <div className="opt-grid">
          {Object.entries(catalog.families).map(([key, f]) => (
            <Swatch
              ...
              title={shortFamily(f)}
              subtitle={`from ${chf(f.base_price || f.sizes?.[0]?.aktion || 0)}`}
      ...
      <Section id="interior" title="Benches & interior" icon={Layers} open={openId === 'interior'}>
        ...
        <div className="switch-grid">
          {[
            ['backrests',    'Backrests'],
            ['sideBackrests','Side backrests'],
            ['apron',        'Under-bench cladding'],
            ['floorGrate',   'Floor grating'],
            ['slidingStool', 'Sliding lower bench'],
          ].map(([key, label]) => (
